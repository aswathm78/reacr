...
export default class BusinessLogin extends Component {
  ...
  componentDidMount() {
    this.show()
    logout();
    window.sessionStorage.setItem('agentName',undefined);
  ...
  handleSubmit = event => {
    ...
    // let encryptedPassword = '123456'; // encrypt(this.state.password);
    let encryptedPassword =this.state.input.password; // encrypt(this.state.password);

  const data ={
        agentRole: "MAALEMUSER",
  ...
  Axios.post('http://122.166.172.240:3031/api/iord-agents/loginAgent', data, config).then(res => {
    console.log(res,"data get")
    if (res.data.id != null && res.data) {
      window.sessionStorage.setItem('agentID', res.data.id);
      window.sessionStorage.setItem('agentName', res.data.firstName + ' ' + res.data.lastName);
      ...
      window.sessionStorage.setItem('agentRole', res.data.agentRole);
      console.log(res.data)
      login()
      this.props.history.push('/business/business-details')
      // this.todashboard();
    } else {
      this.state.lang === 'en'?
  ...
  langToggle =()=>{
    this.state.lang === 'en' ? window.sessionStorage.setItem('language','ar'):window.sessionStorage.setItem('language','en')
    this.state.lang === 'en' ? this.setState({lang:'عربى',display:'right'}):this.setState({lang:'en',display:'left'})
  }
  todashboard = () => {
    this.props.history.push('/business/home');
  };
        ...
        <div className="container" >
          <div className="row">
            <div className="col-sm-12">
              <div className="col-sm-5" style={{ 'margin-top': '16%', textAlign:this.state.display}}>
                <div className="maalme-heading">
              <h3>{this.state.lang=='en'? 'Welcome to Maalem Financing':'مالیام فنانسنگ میں خوش آمدید'}</h3>
              <h3>{this.state.lang=='en'? 'Agent Admin Panel':'لوحة إدارة الوكيل'}</h3>
                </div>
                ...
                <div className="">
                        ...
                        <span style={{fontSize:'1.5rem',color:'white'}}>{this.state.lang=='en'? 'Manage your Agent Accounts':'إدارة حسابات الوكيل الخاص بك'} </span>
              ...
              <div className="col-sm-6 " style={{ 'margin-top': '2%' }}>
                {this.state.activekey === 1 ? (
                  ...
                  <div><button style={{position:'absolute',textAlign:this.state.lang ==='en'? 'right':'left',right:0,backgoundColor:'#206fb7',borderRadius:'.5rem',padding:'.5rem 1rem',margin:'1rem 2rem 0 0'}} onClick={this.langToggle}>{this.state.lang === 'en'? 'عربى':'EN'}</button></div>
                    <div class="card-body">
                      <div className="logintxt">{this.state.lang=='en'? 'Business Sign In':'بزنس سائن ان'}</div>
                      <div className="login_input">
                        <input
                          type="text"
                          placeholder={this.state.lang=='en'? 'Enter Email ID':'أدخل معرف البريد الإلكتروني'}
                          name="email"
                      ...
                      <div id="divPass" className={`login_input ${this.state.display}`}>
                        <input
                          type="password"
                          placeholder={this.state.lang=='en'? 'Password':'پاس ورڈ'}
                          id="password0"
                          name="password"
                          onChange={this.handleChange}
                        />
                      ...
                      <div class="card customerpwd_cardstyle">
                        <div class="card-body">
                          ...
                          <div className="pwd_input">
                            <input type="password" placeholder={this.state.lang=='en'? 'Enter Password':'پاس ورڈ درج کریں'} autoComplete="off" />
                          </div>
